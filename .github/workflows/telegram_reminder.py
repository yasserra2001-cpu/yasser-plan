...
def _normalize_unit(raw):
    """Custom foods entered in the app sometimes carry a bilingual unit dict
    (e.g. {'en': '100g', 'ar': 'جرام'}) instead of a plain string. Flatten to
    a single lowercase string so the '100' substring check still works."""
    if raw is None:
        return ''
    if isinstance(raw, dict):
        return ' '.join(str(v) for v in raw.values() if v).lower()
    ...
    for f in custom_foods or []:
        if f.get('id') == food_id:
            return {'n': f.get('n', ''), 'cal': f.get('cal', 0),
                    'p': f.get('p', 0), 'unit': _normalize_unit(f.get('unit'))}
    return None
...
def item_totals(item, custom_foods):
    ...
    if not f:
        return {'cal': 0, 'p': 0}
    q = item.get('q', 0)
    unit = _normalize_unit(f.get('unit'))
